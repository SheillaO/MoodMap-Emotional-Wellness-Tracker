...
function getMatchingCatsArray() {
    ...
    const selectedEmotionEl = document.querySelector('input[name="emotions"]:checked')
...
function renderMoodHistory() {
    ...
    if (history.length === 0) {
        moodHistoryContainer.innerHTML = '<p class="empty-state">No mood entries yet. Track your first emotion!</p>'
        return
    }
    ...
    moodHistoryContainer.innerHTML = recentEntries.map((entry, index) => `
...
function filterByEmotion(emotion) {
    ...
    moodHistoryContainer.innerHTML = filtered.map(entry => `
        <div class="mood-card">
